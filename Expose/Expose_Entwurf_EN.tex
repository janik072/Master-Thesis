...
\begin{itemize}
    \item \textbf{Waterfall cost experiment (central)}: Incremental activation of optimization strategies on a fixed baseline scenario. Each stage measures cost reduction (\%) and quality delta relative to the unoptimized baseline:
    \begin{enumerate}[label=Stage~\arabic*:, leftmargin=2cm]
        \item Baseline (full context, GPT-4o, free-text output, no caching)
        \item + Structured output (JSON mode)
        \item + Prompt compression (sliding window / history summarization)
        \item + Model routing (cheap model for simple rounds)
        \item + Response caching (SHA-256 prompt hashing)
        \item + Local models (Ollama) for selected sub-tasks
    \end{enumerate}
    Result: an empirical \textbf{cost--quality tradeoff curve} across $N{=}10$ repetitions per stage.
    \item \textbf{Model comparison}: Same scenario across GPT-4o, GPT-4o-mini, Claude Sonnet, Mistral Large, and Llama-3 (local). Cost, quality, and latency per model.
    \item \textbf{Application scenario}: Peatland rewetting negotiation with diverse farmer personas and incentive mechanisms. Plausibility analysis of agent decisions.
    \item \textbf{Reproducibility test}: 30 runs with identical configuration at varying temperature levels ($t{=}0.0, 0.3, 0.7$). Statistical evaluation of result variance.
    \item \textbf{(Optional) Benchmark validation}: Ultimatum Game and Prisoner's Dilemma with known equilibria as a sanity check.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{enumerate}[label=\arabic{enumi}.\arabic*]
        ...
        \item Waterfall cost experiment: cumulative optimization impact
        \item Cost--quality tradeoff analysis
        \item Model comparison: cost-performance across providers
        \item Use case: peatland rewetting simulation
        \item Reproducibility and sensitivity analysis
...
\begin{enumerate}
    ...
    \item \textbf{Scientific contribution}: Empirical cost--quality tradeoff analysis across five optimization strategies, quantifying their individual and cumulative impact on simulation cost and result quality (waterfall experiment).
    \item \textbf{Methodological contribution}: Design patterns and best practices for reproducible, cost-efficient LLM-based experimental simulations, including a declarative persona schema and configurable optimization pipeline.
...
\bibitem{teubner2025}
Teubner, T., Flath, C.\,M. \& Weinhardt, C. (2025).
LLM-Based Agents in Economic Experiments: Methodological Insights from Gift-Exchange Games.
\textit{Working Paper}, Paderborn University.
